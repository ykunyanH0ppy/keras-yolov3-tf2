from tensorflow.keras.layers import (Conv2D, Input, BatchNormalization,
                                     LeakyReLU, ZeroPadding2D, UpSampling2D, Lambda, Add, Concatenate)
...
class YoloLayer(Layer):
    ...
    def call(self, x):
        ...
        class_mask  = tf.expand_dims(tf.cast(tf.equal(tf.argmax(input=pred_box_class, axis=-1), true_box_class), tf.float32), 4)
        recall50    = tf.reduce_sum(input_tensor=tf.cast(iou_scores >= 0.5, tf.float32) * detect_mask  * class_mask) / (count + 1e-3)
        recall75    = tf.reduce_sum(input_tensor=tf.cast(iou_scores >= 0.75, tf.float32) * detect_mask  * class_mask) / (count + 1e-3)
        ...
        if debug:
            loss = tf.compat.v1.Print(loss, [grid_h, avg_obj], message='avg_obj \t\t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [grid_h, avg_noobj], message='avg_noobj \t\t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [grid_h, avg_iou], message='avg_iou \t\t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [grid_h, avg_cat], message='avg_cat \t\t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [grid_h, recall50], message='recall50 \t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [grid_h, recall75], message='recall75 \t', summarize=1000)   
            loss = tf.compat.v1.Print(loss, [grid_h, count], message='count \t', summarize=1000)     
            loss = tf.compat.v1.Print(loss, [grid_h, tf.reduce_sum(input_tensor=loss_xy), 
                                        tf.reduce_sum(input_tensor=loss_wh), 
                                        tf.reduce_sum(input_tensor=loss_conf), 
                                        tf.reduce_sum(input_tensor=loss_class)],  message='loss xy, wh, conf, class: \t',   summarize=1000)   
